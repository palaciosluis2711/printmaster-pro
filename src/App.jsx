...
// --- MÓDULOS AISLADOS (CANVAS & VISTAS) ---
import { HomeDashboard } from './modules/home';
import { GridCanvas } from './modules/grid';
import { CustomGridCanvas } from './modules/custom-grid';
import { MosaicCanvas } from './modules/mosaic';
import { BannerCanvas } from './modules/banner';
import { CVCanvas, CVLanding } from './modules/cv';
...
export default function App() {
  ...
  const isEditingCV = activeView === 'cv' && Number(studio.config.step || 0) > 0;
  const showCanvas = ['grid', 'mosaic', 'custom', 'banner'].includes(activeView) || isEditingCV;

  // --- PREVENCIÓN DE ZOOM DEL NAVEGADOR & ZOOM EXCLUSIVO PARA EL CANVAS ---
  useEffect(() => {
    const handleGlobalWheel = (e) => {
      // Bloquear zoom nativo del navegador con Ctrl + rueda / pinch gesture
      if (e.ctrlKey) {
        e.preventDefault();
        
        // Si el puntero está sobre el canvas activo, aplicar zoom al lienzo
        const canvasElement = e.target.closest('main');
        if (canvasElement && showCanvas) {
          if (e.deltaY < 0) {
            // Rueda hacia arriba -> Acercar (In)
            studio.setZoom(z => Math.min(3.0, parseFloat((z + 0.1).toFixed(1))));
          } else if (e.deltaY > 0) {
            // Rueda hacia abajo -> Alejar (Out)
            studio.setZoom(z => Math.max(0.2, parseFloat((z - 0.1).toFixed(1))));
          }
        }
      }
    };
    ...
    const handleGlobalKeyDown = (e) => {
      // Bloquear atajos nativos de zoom de página en el navegador (Ctrl +, Ctrl -, Ctrl 0, Ctrl =)
        ...
        {showCanvas ? (
          ...
          <>
            {activeView === 'home' && (
              <HomeDashboard
                favorites={studio.favorites}
                onLoadFavorite={loadFavoriteFromDashboard}
              />
            )}
            ...
            {!['home', 'cv'].includes(activeView) && (
              <div className="flex-1 bg-slate-200/50 flex flex-col items-center justify-center text-slate-400 p-8">
