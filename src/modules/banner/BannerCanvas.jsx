...
export default function BannerCanvas({
    ...
    const layout = useMemo(() => {
        ...
        if (config.avoidCuts) {
            ...
            chars.forEach(char => {
                ...
                const endPage = Math.floor((currentX + charW - 0.1) / contentWidthPx);
        ...
        return {
            ...
            bannerTotalWidthPx, fullLineHeightPx,
    ...
    // Report totalPages and layout to parent
    useEffect(() => {
                ...
                {isBannerPreview ? (
                    <div className="flex flex-col items-center justify-center w-full min-h-full p-4 animate-in fade-in zoom-in-95 duration-300">
                        ...
                        <div className="mt-6 bg-slate-800 text-white px-5 py-2.5 rounded-full shadow-xl text-xs font-bold flex items-center gap-3 z-20 sticky bottom-4 backdrop-blur-md bg-slate-800/90 border border-slate-600">
                            <span>
                                Tamaño: <span className="text-pink-300 text-sm">{Math.round(layout.colsP * layout.pageWidthMm)} x {Math.round(layout.rowsP * layout.pageHeightMm)} mm</span>
                            </span>
                            <span className="w-px h-4 bg-slate-600 mx-1"></span>
                            <span><span className="text-pink-300">{layout.colsP} x {layout.rowsP}</span> Hojas</span>
                        </div>
                    </div>
                ) : (
                    Array.from({ length: layout.totalPages }).map((_, pageIndex) => {
                        ...
                        return (
                            ...
                            >
                                <div
                                    className="bg-white shadow-xl print-no-zoom-inner transition-transform duration-300 origin-top-left print:shadow-none print:m-0 relative overflow-hidden"
                                    style={{
                                ...
                                >
                                    <div className="w-full h-full relative overflow-hidden bg-slate-50/10 border border-slate-100/50">
                                        {config.printGuides && (
                                            <div className="absolute inset-0 border border-pink-200 pointer-events-none z-10"></div>
                                        )}
