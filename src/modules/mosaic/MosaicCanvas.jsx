...
export default function MosaicCanvas({
    ...
    const layout = useMemo(() => {
        const pageSize = PAGE_SIZES[config.pageSize] || PAGE_SIZES.carta;
        ...
        if (mosaicImage) {
            if (config.mosaicType === 'pieces') {
                colsP = config.mosaicCols || 1;
                rowsP = config.mosaicRows || 1;
            } else {
                colsP = Math.ceil((config.mosaicTargetWidth || contentWidth) / contentWidth);
                rowsP = Math.ceil((config.mosaicTargetHeight || contentHeight) / contentHeight);
            }
        ...
        const scaleW = availW / (totalW_px || 1);
        const scaleH = availH / (totalH_px || 1);
        ...
        if (mosaicImage) {
            if (config.mosaicType === 'size') {
                ...
                const containerRatio = mosaicTotalW_mm / (mosaicTotalH_mm || 1);
                ...
                const imgRatio = (imgNatW || 1) / (imgNatH || 1);
    ...
    // Helper para renderizar cada página del mosaico
                        ...
                        {(config.mosaicType === 'pieces' && config.mosaicCols === 1 && config.mosaicRows === 1) && (
                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors print:hidden flex items-start justify-end p-2 gap-1 opacity-0 group-hover:opacity-100 pointer-events-none">
                                <div className="pointer-events-auto flex gap-1">
                                    <button onClick={onToggleMosaicFit} className="bg-white text-slate-700 p-1.5 rounded shadow-sm hover:bg-blue-50 hover:text-blue-600 transition cursor-pointer"><Scaling className="w-4 h-4" /></button>
                                    <button onClick={onRotateMosaic} className="bg-white text-slate-700 p-1.5 rounded shadow-sm hover:bg-blue-50 hover:text-blue-600 transition cursor-pointer"><RotateCw className="w-4 h-4" /></button>
                                    <button onClick={onRemoveMosaic} className="bg-white text-red-500 p-1.5 rounded shadow-sm hover:bg-red-50 transition cursor-pointer"><X className="w-4 h-4" /></button>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className={`flex flex-col gap-8 print:gap-0 shrink-0 items-center p-0 print:p-0 print:block print:h-auto ${isMosaicPreview ? 'min-h-full justify-center w-full' : ''}`}>
                ...
                {isMosaicPreview || !mosaicImage ? (
                    <div className="flex flex-col items-center justify-center w-full min-h-full p-4 animate-in fade-in zoom-in-95 duration-300">
                        <div
                            className={`bg-white shadow-2xl relative border transition-all duration-150 ease-out origin-center flex items-center justify-center ${!mosaicImage ? 'border-2 border-dashed border-slate-300 hover:border-purple-400 hover:bg-slate-50 cursor-pointer' : 'border-slate-400'}`}
                            style={{
                        ...
                        >
                            {!mosaicImage ? (
                                <div className="text-center p-6 text-slate-400 flex flex-col items-center select-none">
                                    ...
                                    <h3 className="text-xl font-bold text-slate-600 mb-1">Cargar Imagen para Mosaico</h3>
                                    <p className="text-xs text-slate-400 max-w-[220px]">Haz clic para cargar una imagen y dividirla en múltiples hojas</p>
                                </div>
                            ) : (
                                <>
                    ...
                    Array.from({ length: layout.totalPages }).map((_, pageIndex) => (
                        <div
                            key={pageIndex}
                            className={`page-wrapper print-no-zoom-outer relative shrink-0 print:block print:overflow-visible ${pageIndex < layout.totalPages - 1 ? 'print:break-after-page' : ''}`}
                            style={{ width: `${layout.pageWidthMm * zoom}mm`, height: `${layout.pageHeightMm * zoom}mm` }}
                        >
                            <div
                            ...
                            <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 text-xs text-slate-400 font-bold print:hidden">
                                Página {pageIndex + 1} de {layout.totalPages}
                            </div>
                        </div>
                    ))
                )}
