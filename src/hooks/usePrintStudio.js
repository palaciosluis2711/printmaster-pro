...
export function usePrintStudio() {
  ...
  const saveCVDraft = (name, cvConfig) => {
    const existingId = cvConfig?.activeDraftId;
    let savedId = existingId;
    
    const currentSnapshot = JSON.stringify({
      personalData: cvConfig.personalData || {},
      pageSize: cvConfig.pageSize || 'carta'
    });

    setCVDrafts(prev => {
      if (existingId && prev.some(d => d.id === existingId)) {
        return prev.map(d =>
          d.id === existingId
            ? {
                ...d,
                ...
                config: { ...cvConfig, lastSavedSnapshot: currentSnapshot }
              }
            : d
        );
      }
      ...
      const draft = {
        ...
        date: new Date().toISOString(),
        config: { ...cvConfig, activeDraftId: newId, lastSavedSnapshot: currentSnapshot }
      };
      return [draft, ...prev];
    });
